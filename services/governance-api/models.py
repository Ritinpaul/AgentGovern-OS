...
class Agent(Base):
    ...
    dna_profile: Mapped[dict] = mapped_column(JSON, default=dict)
    social_contract: Mapped[dict] = mapped_column(JSON, default=dict)
    platform_bindings: Mapped[list] = mapped_column(JSON, default=list)
...
class AgentGene(Base):
    ...
    source_agent_id: Mapped[uuid.UUID | None] = mapped_column(Uuid, ForeignKey("agents.id"), nullable=True)
    ...
    mutation_log: Mapped[list] = mapped_column(JSON, default=list)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now())
...
class Decision(Base):
    ...
    output_action: Mapped[dict] = mapped_column(JSON, nullable=False)
    ...
    policy_rules_applied: Mapped[list] = mapped_column(JSON, default=list)
    policy_violations: Mapped[list] = mapped_column(JSON, default=list)
    sentinel_assessment: Mapped[dict | None] = mapped_column(JSON, nullable=True)
    prophecy_paths: Mapped[dict | None] = mapped_column(JSON, nullable=True)
    human_override: Mapped[dict | None] = mapped_column(JSON, nullable=True)
    outcome_feedback: Mapped[dict | None] = mapped_column(JSON, nullable=True)
...
class TrustEvent(Base):
    ...
    trigger_decision_id: Mapped[uuid.UUID | None] = mapped_column(Uuid, nullable=True)
    ...
    authority_change: Mapped[dict | None] = mapped_column(JSON, nullable=True)
...
class Policy(Base):
    ...
    applies_to_roles: Mapped[list] = mapped_column(JSON, default=lambda: ["*"])
    applies_to_tiers: Mapped[list] = mapped_column(JSON, default=lambda: ["*"])
...
class SocialContract(Base):
    ...
    status: Mapped[str] = mapped_column(String(20), default="active")
    rights: Mapped[dict] = mapped_column(JSON, nullable=False)
    responsibilities: Mapped[dict] = mapped_column(JSON, nullable=False)
    authority_bounds: Mapped[dict] = mapped_column(JSON, nullable=False)
    career_path: Mapped[dict] = mapped_column(JSON, nullable=False)
    review_schedule: Mapped[dict] = mapped_column(JSON, nullable=False)
    mentor_assignments: Mapped[list] = mapped_column(JSON, default=list)
    performance_benchmarks: Mapped[dict] = mapped_column(JSON, nullable=False)
...
class EscalationCase(Base):
    ...
    prophecy_recommendation: Mapped[dict | None] = mapped_column(JSON, nullable=True)
    ...
    feedback_to_agent: Mapped[dict | None] = mapped_column(JSON, nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
