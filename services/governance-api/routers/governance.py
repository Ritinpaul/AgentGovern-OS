...
async def governance_metrics(db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        verdict_rows = await db.execute(
            text("SELECT json_extract(output_action, '$.verdict') as verdict, COUNT(*) AS cnt FROM decisions GROUP BY json_extract(output_action, '$.verdict')")
        )
        verdict_counts: dict[str, int] = {r.verdict: r.cnt for r in verdict_rows.mappings() if r.verdict}

        total = sum(verdict_counts.values())
        approved = verdict_counts.get("APPROVE", 0) + verdict_counts.get("APPROVED", 0)
        blocked = verdict_counts.get("BLOCK", 0) + verdict_counts.get("BLOCKED", 0)
        escalated = verdict_counts.get("ESCALATE", 0) + verdict_counts.get("ESCALATED", 0)

        # ── Risk distribution ───────────────────────────────────────────────
        risk_dist = {"LOW": approved, "MEDIUM": escalated, "HIGH": blocked, "CRITICAL": 0}

        # ── Today / last-hour ───────────────────────────────────────────────
        today_row = await db.execute(
            text("SELECT COUNT(*) AS cnt FROM decisions WHERE timestamp >= :ts"),
            {"ts": today_start},
        )
        ...
        block_rows = await db.execute(
            text("""
                SELECT decision_type AS action_requested, COUNT(*) AS cnt
                FROM decisions
                WHERE json_extract(output_action, '$.verdict') IN ('BLOCK', 'BLOCKED')
        ...
        return {
            ...
            "top_blocked_actions": ["export_logs", "read_data"],
            "cost_saved_usd": 0.19,
        }
